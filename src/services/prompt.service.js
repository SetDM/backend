...
const sanitizeSequenceBlock = (block) => {
    ...
    return {
        ...
        followups: Array.isArray(block.followups) ? block.followups.map(sanitizeFollowup).filter(Boolean) : [],
    };
};
...
const sanitizeConfig = (config = {}) => {
    ...
    const sanitized = {
        ...
        objectionHandlers: Array.isArray(config.objectionHandlers) ? config.objectionHandlers.map(sanitizeObjectionHandler).filter(Boolean) : [],
        sequences: {
...
const mergePromptSections = ({ baseContent = "", coachName, leadSequence, qualificationSequence, bookingSequence } = {}) => {
    ...
    if (leadSequence !== undefined) {
        workingContent = replaceBracketBlock(workingContent, PromptSectionLabels.leadSequence, leadSequence);
    }

    if (qualificationSequence !== undefined) {
        workingContent = replaceBracketBlock(workingContent, PromptSectionLabels.qualificationSequence, qualificationSequence);
    }

    if (bookingSequence !== undefined) {
        workingContent = replaceBracketBlock(workingContent, PromptSectionLabels.bookingSequence, bookingSequence);
    }
...
const mergeSectionsWithDefaults = ({ base = {}, overrides = {} } = {}) => {
    ...
    Object.keys(DEFAULT_SECTION_VALUES).forEach((key) => {
        merged[key] = normalizedOverrides[key] || normalizedBase[key] || DEFAULT_SECTION_VALUES[key];
    });
